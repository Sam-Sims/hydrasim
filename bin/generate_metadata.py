...
def main() -> None:
    ...
    parser.add_argument(
        "--badread-per-segment", action=argparse.BooleanOptionalAction, required=True
    )
    ...
    parser.add_argument(
        "--downsample-background", action=argparse.BooleanOptionalAction, required=True
    )
    ...
    for run_index, record in enumerate(records):
        if record["single_end"]:
            ...
            methods = {
                ...
                "input_accession": record["accession"],
                "simulator": simulator,
                ...
                "simulated_mean_read_identity": record["simulated_mean_read_identity"],
            }
        else:
            strategy = "wgsim"
            simulator = "wgsim"
        ...
        writer.writerow(
            {
                ...
                "source_climb_id": record["source_climb_id"],
                "applications": json.dumps(
                    [f"hydrasim {args.hydrasim_version}", simulator]
                ),
